...
case "$choice" in
	1 ) target="L5-"; variant="NFC"; defconfig="skernel_m4_defconfig";;
	2 ) target="L5-"; variant="NoNFC"; defconfig="skernel_m4_nonfc_defconfig";;
	3 ) target="L7-"; variant="NFC"; defconfig="skernel_u0_defconfig";;
	4 ) target="L7-"; variant="NoNFC"; defconfig="skernel_u0_nonfc_defconfig";;
	5 ) target="L1II-"; variant="SD"; defconfig="skernel_v1_defconfig";;
	6 ) target="L3II-"; variant="SD"; defconfig="skernel_vee3_defconfig";;
...
echo "${bldblu}Building $customkernel $version with $NR_CPUS jobs at once${txtrst}"
...
if [ "$maindevicecheck" == "" ]; then
	...
	if [ "$CROSS_COMPILE" == "" ]; then
		...
		echo "5) Build $customkernel $version (${bldyel}$buildprocesscheck${txtrst})"
	fi
fi
if [ -f arch/$ARCH/boot/zImage ]; then
